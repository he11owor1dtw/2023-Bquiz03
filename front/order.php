  <div id='select' style="display:flex;justify-content:center;margin-top:80px">
    <div class="order">
      <div>
        <label>電影：</label>
  ...
  <script>
    ...
    function booking() {
      let order={movie_id:$("#movie").val(),date:$("#date").val(),session:$("#session").val()}
      $.get("./api/booking.php",order, (booking) => { // 先去後端抓資料
        $('#booking').html(booking) // 印出來      
        $('#select').hide();
        $('#booking').show()
      })
